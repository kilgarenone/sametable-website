...
- **Client-side rendering(CSR); SPA; JSON APIs** &mdash;
  This is perhaps the most popular approach. It's great for building interactive web applications. But [be aware](https://macwright.org/2020/05/10/spa-fatigue.html) of its downsides and steps to mitigate them. This is the approach I took, so we will talk about this in much details.
...
- **Server-side rendering and 'sprinkle ✨ it with Javascript'**
  &mdash; This was the old-school way of building on the web!&mdash;Use PHP to build your templates with data in your server, then bind events handlers to the DOM with jQuery in the browser. This approach might have been ill-suited to build the increasingly complex app that businesses have asked for on the web, but some technologies have emerged to warrant a reconsideration:
